...
fn find_guard_who_has_slept_the_most(
    minutes_spent_asleep_per_guard: &HashMap<GuardId, Vec<Minute>>,
) -> GuardId {
    let (guard_id, _) =
        minutes_spent_asleep_per_guard
            .keys()
            .fold((0, 0), |(guard, minutes), curr_guard| {
                let minutes_spent_asleep = minutes_spent_asleep_per_guard
                    .get(curr_guard)
...
fn get_minute_guard_was_asleep_most_often(
    minutes_spent_asleep_per_guard: &HashMap<GuardId, Vec<Minute>>,
    guard_id: GuardId,
) -> Minute {
    let mut num_times_each_minute_was_spent_asleep: HashMap<Minute, u32> = HashMap::new();

    let all_minutes_guard_spent_asleep: &Vec<Minute> = minutes_spent_asleep_per_guard
        .get(&guard_id)
        .expect("key must be present");
    for minute in all_minutes_guard_spent_asleep {
        *num_times_each_minute_was_spent_asleep
            .entry(*minute)
            .or_insert(0) += 1;
    }
...
fn get_most_slept_minute(num_times_each_minute_was_spent_asleep: &HashMap<Minute, u32>) -> Minute {
    let (minute, _) = num_times_each_minute_was_spent_asleep.keys().fold(
        (0, 0),
        |(min, amt_slept), curr_minute| {
            let times_asleep = num_times_each_minute_was_spent_asleep
                .get(curr_minute)
                .expect("key must be present");
